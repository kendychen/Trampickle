...
func ThemDichVu(o ODichVu) error {
	dvMu.Lock()
	defer dvMu.Unlock()
	if !reMaDV.MatchString(o.Ma) {
		return fmt.Errorf("ma %q chi gom A-Z, 0-9, _, 2-24 ky tu", o.Ma)
	}
	for _, d := range GIA.DichVu {
		if d.Ma == o.Ma {
			return fmt.Errorf("ma %s da co", o.Ma)
		}
	}
	if o.Ten == "" {
		return fmt.Errorf("chua dien ten viec")
	}
	if o.DoiTuong == "" {
		o.DoiTuong = "vot"
	}
	if !LaDoiTuongHopLe(o.DoiTuong) {
		return fmt.Errorf("doi tuong phai la vot, giay hoac ca_hai")
	}
	b, err := os.ReadFile(fileBangGia())
	if err != nil {
		return err
	...
	dau, cuoi := khoiKhoaTrenCung(dong, "dich_vu")
	if dau < 0 {
		return fmt.Errorf("khong tim thay khoi dich_vu")
	}
	chen := cuoi
	for chen > dau+1 && strings.TrimSpace(dong[chen-1]) == "" {
		chen--
	}
	...
	if o.DoiTuong != "" && o.DoiTuong != "vot" {
		moiDong = append(moiDong, "    doi_tuong: "+o.DoiTuong)
	}
	moiDong = append(moiDong, "    "+giaDong)
	has := false
	for _, v := range o.GiaDen {
		if v != "" {
			has = true
			break
		}
	...
	if has {
		moiDong = append(moiDong, "    "+dongGia("gia_den", o.GiaDen))
	}
	moiDong = append(moiDong, "    vat_tu: 0")
	moiDong = append(moiDong, "    gio_cong: 0")
	if o.BaoHanhThang != "" && o.BaoHanhThang != "0" {
		moiDong = append(moiDong, "    bao_hanh_thang: "+o.BaoHanhThang)
	}
	if o.LeadTimeNgay != "" && o.LeadTimeNgay != "0" {
		moiDong = append(moiDong, "    lead_time_ngay: "+o.LeadTimeNgay)
	}
	if o.LeadTimeDen != "" {
		moiDong = append(moiDong, "    lead_time_ngay_den: "+o.LeadTimeDen)
	}
	if o.ThuTu != "" && o.ThuTu != "0" {
		moiDong = append(moiDong, "    thu_tu: "+o.ThuTu)
	}
	if o.TuGiaiDoan > 1 {
		moiDong = append(moiDong, "    tu_giai_doan: "+strconv.Itoa(o.TuGiaiDoan))
	}
	if o.DieuKien != "" {
		moiDong = append(moiDong, dongFolded("dieu_kien", o.DieuKien)...)
	}
	if o.BaoGiaRieng {
		moiDong = append(moiDong, "    bao_gia_rieng: true")
	}
	if o.NoiBat {
		moiDong = append(moiDong, "    noi_bat: true")
	}
	if o.An {
		moiDong = append(moiDong, "    an: true")
	}
	ra := make([]string, 0, len(dong)+len(moiDong))
	ra = append(ra, dong[:chen]...)
	ra = append(ra, moiDong...)
	ra = append(ra, dong[chen:]...)
	out := strings.Join(ra, "\n")
	var thu BangGia
	if err := yaml.Unmarshal([]byte(out), &thu); err != nil {
		return fmt.Errorf("them xong bang gia khong doc duoc: %w", err)
	}
	if err := ghiAtomic(fileBangGia(), []byte(out)); err != nil {
		return err
	}
	GIA.DichVu = thu.DichVu
	return nil
...
func XoaDichVu(ma string) error {
	...
	if !found {
		return fmt.Errorf("khong thay %s", ma)
	}
	b, err := os.ReadFile(fileBangGia())
	if err != nil {
		return err
	...
	if ds < 0 {
		return fmt.Errorf("khong thay muc %s trong file", ma)
	}
	ra := make([]string, 0, len(dong)-(de-ds))
	ra = append(ra, dong[:ds]...)
	ra = append(ra, dong[de:]...)
	out := strings.Join(ra, "\n")
	var thu BangGia
	if err := yaml.Unmarshal([]byte(out), &thu); err != nil {
		return fmt.Errorf("xoa xong bang gia hong: %w", err)
	}
	if len(thu.DichVu) != len(GIA.DichVu)-1 {
		return fmt.Errorf("sau khi xoa con %d thay vi %d", len(thu.DichVu), len(GIA.DichVu)-1)
	}
	if err := ghiAtomic(fileBangGia(), []byte(out)); err != nil {
		return err
	}
...
func hQtDichVu(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Get("ok") != "" && d.OK == "" {
		d.OK = r.URL.Query().Get("ok")
	}
	if r.URL.Query().Get("loi") != "" && d.Loi == "" {
...
func hQtDichVuThem(w http.ResponseWriter, r *http.Request) {
	...
	if v := strings.TrimSpace(r.FormValue("thu_tu")); v != "" {
		o.ThuTu = v
	}
	if v := strings.TrimSpace(r.FormValue("dieu_kien")); v != "" {
		o.DieuKien = v
	}
	...
	m, err := docODichVu(base, func(k string) string {
		...
		case "bao_hanh_thang", "lead_time_ngay", "lead_time_ngay_den", "thu_tu":
			...
			return ""
		case "bao_gia_rieng", "noi_bat", "an":
			return ""
		default:
			// gia/giaden
			for i := 0; i < 3; i++ {
				if k == "gia"+strconv.Itoa(i+1) {
					return gia[i]
				}
				if k == "giaden"+strconv.Itoa(i+1) {
					return giaDen[i]
				}
			}
			return ""
	...
	o.Ten = m.Ten
	o.DieuKien = m.DieuKien
	o.DoiTuong = m.DoiTuong
	o.Gia = m.Gia
	o.GiaDen = m.GiaDen
	o.ThuTu = m.ThuTu
	o.Nhom = nhom
...
func hQtDichVuXoa(w http.ResponseWriter, r *http.Request) {
	...
	if !reMaDV.MatchString(ma) {
		http.Error(w, "mã không hợp lệ", 400)
		return
	}
